  ...
  const handlePlay = async (card) => {
    try {
      ...
      const requestBody = JSON.stringify({
        ...
        aRank: selectedCard.aRank,
        aOption: selectedCard.aOption
      });
    ...
    <div className="player-hand">
      {cards.map((card, index) => (
        <div key={index} className="player-hand-card-wrapper"
          ...
          <Card
            className={`player-hand-card ${selectedCard === card ? 'selected' : ''} ${!card.playable ? 'unplayable' : ''}`}
            path={getImagePath(card)} onClick={() => handleCardClick(card, index)}
      ...
      {selectedCard && (
        <div className="selected-card-buttons" style={selectedCardWrapperStyle}>
          ...
          <button className="selected-card-button cancel-button" style={cancelButtonStyle}
            onClick={handleCancel}>Cancel
          </button>
          <button className="selected-card-button play-button" style={playButtonStyle} onClick={handlePlay}
            disabled={recentRequest}>Play
          </button>
        </div>
      )}
      {scarryMarry && (
